...
**Path-scoped pattern — `bare-runtime-bash-lib` (ORI-242):** under `agents/` and `references/` only, bare runtime invocations of the form `bash lib/...` fail the scan. Runtime docs must use `bash {skill-root}/lib/...` after Load Config resolves `$SKILL_ROOT`. **Allowlist (skill-dev regression gates only):** `bash lib/tests/...` and `bash lib/conformance-scan.sh` (tracker docs / CONTRIBUTING). Catalog identity `lib/*.sh` without a leading `bash ` is not matched. This pass is separate from the default roots so `references/` is not pulled into unrelated patterns.
