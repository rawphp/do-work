...
**Hierarchy lock (authoritative):** do-work **Issue** (`UR-NNN`) = **Project Milestone** on shared `product_project` (default `do-work`). **Not** Initiative-as-do-work-Issue. REQs = Linear issues (different entity). Path narratives below may still mention historical Initiative wording in child-work tables; prefer the lock + [linear-ops.md](linear-ops.md) sequences.
