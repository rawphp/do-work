...
Milestone mode is **implicit** — triggered by Issue shape, not a flag. Issues that do not match the trigger continue to behave as before. The `/saas-thesis` skill produces Issue files with the correct shape for handoff.
