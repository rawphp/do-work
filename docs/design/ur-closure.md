...
| REQ-213 | agents | `lib/coverage-rollup.sh` (+ `lib/coverage-rollup.test.sh`) | Add the additive end-to-end closure column read from `UR-NNN/closure.md`; extend the test to cover the `closure=none` / `closed=N gaps=N` / `no-path-units` cases. |
