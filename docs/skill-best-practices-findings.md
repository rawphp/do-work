...
| F15 | **Markdown historical trees after migrate are easy to re-activate by mistake** | **major** | Post-cutover: local Issue/REQ trees remain as **read-only history** while `backend: linear`. SKILL install/list steps and many agents still name those paths. Conformance/upgrade document ignore rules; a rushed agent can still `ls .do-work/` and treat files as live. | **docs** + later **code** (conformance detector / guardrails already partial — extend messaging in SKILL stubs). |
