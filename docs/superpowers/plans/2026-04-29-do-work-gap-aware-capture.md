...
**Cleanup scope.** This removes only the synthetic UR and its REQ files. The `do-work/config.yml` declared in Task 26 stays committed — that's the project's actual dogfood configuration, not a smoke-test artifact.
