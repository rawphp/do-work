...
**Linear hierarchy:** **do-work Issue (slug UR-NNN) = Project Milestone** on a **shared product Project** per local product (`tracker.linear.product_project` — name or UUID; **default empty**). Resolve: explicit `product_project` → `project.name` → git-root basename; `ensure_product_container` create-if-missing + **always persist UUID**. Never invent skill name `do-work` for empty config (example name for this skill repo only). REQs = **Linear issues** on that milestone (not do-work Issues). Not Initiatives (MCP has no Initiative create tools). When both nouns appear, write **do-work Issue** vs **Linear issue (REQ)** — never "Issue's Issues".
