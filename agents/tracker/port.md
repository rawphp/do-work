...
In Linear mode these live only in Linear (Initiatives, Projects, Issues, Docs, comments) — **no dual-write** to Issue/REQ markdown as source of truth.
...
| | |
|---|---|
| **Intent** | Record a new intake brief as an Issue. |
| **Preconditions** | `ensure_product_container` satisfied; next Issue slug allocatable; backend store writable. |
...
| **Preconditions** | UR exists; ideate phase allowed for that Issue. |
