...
- Read Issue + REQs via **`read_ur`** / **`list_reqs_for_ur`** / **`read_req`** (`agents/tracker/do-work-io.md`) — never glob `user-requests/` or `REQ-*.md` as the live store
- Auto-fix REQ field edits via **`update_req`** / **`set_files`** / AC ops only — never dual-write markdown REQ files
- UI evidence paths: `.do-work/evidence/UR-NNN/ui-evidence/` (not `user-requests/…/ui-evidence/`)
