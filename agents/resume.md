...
- If **no match**: report `"REQ-NNN is not in working/ — nothing to resume."` and stop. Resume only operates on in-flight REQs; do not search `archive/` or the backlog root.
